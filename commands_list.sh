# Build app images
docker build --tag flask_docker_class --tag flask_docker_class:1.0 --file Dockerfile.prod 
...
docker run --rm --detach --network bridge --publish 5432:5432 --name postgresql --env POSTGRES_PASSWORD=flask --env POSTGRES_USER=flask --env POSTGRES_DB=course postgres
...
docker run --rm --detach --network bridge --publish 5432:5432 --volume docker_course:/var/lib/postgresql/data --name postgresql --env POSTGRES_PASSWORD=flask --env POSTGRES_USER=flask --env POSTGRES_DB=course postgres
psql --user flask --dbname course --host 172.17.0.2 < postgresql/data.sql
psql --user flask --dbname course --host 172.17.0.2 --command "SELECT * FROM public.user;"
docker stop postgresql
docker run --rm --detach --network bridge --publish 5432:5432 --volume docker_course:/var/lib/postgresql/data --name postgresql --env POSTGRES_PASSWORD=flask --env POSTGRES_USER=flask --env POSTGRES_DB=course postgres
